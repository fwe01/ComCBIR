...
def create_class_datasets(dataset):
    ...
    for class_num in range(NUM_CLASS):
        class_dataset =[(a, b) for a, b in dataset if (np.argmax(b) == class_num)]
...
def save_model(model, model_name, dataset, feature_num=NUM_FEATURES, epoch=EPOCH, margin=10):
    model_filename = f"automated_classification_{dataset}_{model_name}_updated_negative_loss_function_{str(margin).replace('.', '_')}_{ACTIVATIONS}_{feature_num}_{str(LEARNING_RATE).replace('.', '_')}_{BATCH_SIZE}_{epoch}.h5"
    model_path = f"../Model/{model_filename}"

    if not OVERIDE and os.path.exists(model_path):
...
def train_model(model, class_datasets, epoch=EPOCH, margin=10, verbose=False):
    loss_class = CategoricalCrossEntropyLoss()
...
def worker(identifier, model_name, num_feature, margin, dataset):
    ...
    train_model(model, class_datasets, epoch=50, margin=margin)
    save_model(model, model_name, dataset, num_feature, epoch=50, margin=margin)
...
if __name__ == "__main__":
    ...
    datasets = [
        # 'kvasir_scaled_min1_to_1_classification',
        'dermamnist_scaled_min1_to_1_classification',
    ]
